...
yeni_sutunlar_str = st.text_input("Tablo Sütunları (Virgülle ayırın):", ", ".join(st.session_state.veri_df.columns))
...
# ==========================================
# ÇIKTI MOTORLARI (TAM OTOMATİK ANTETLİ)
# ==========================================
...
def excel_olustur(dataframe, g_str, tarih, sablon, gizle):
    ...
    birim_sutun = get_birim_col(dataframe.columns)
    row_idx = 11 # Üst boşluk
    ws.cell(row=row_idx, column=2, value=f"{sablon} - {tarih}").font = Font(bold=True)
    row_idx += 2
    ...
    ws.cell(row=row_idx+1, column=len(headers)+1, value=f"Toplam: {g_str}").font = Font(bold=True)
    bio = io.BytesIO()
    wb.save(bio)
    return bio.getvalue()
...
def pdf_olustur(dataframe, a_str, k_str, g_str, tarih, notlar, kur_m, sablon, gizle):
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.set_y(70) # Logodan kurtulmak için
            if self.page_no() == 1:
                self.set_font('Arial', 'B', 15)
                self.cell(0, 10, cevir_tr(sablon.upper()), 0, 1, 'C')

    pdf = PDF()
    pdf.set_margins(left=30, top=70, right=30) # Daraltılmış ve ortalanmış
    pdf.add_page()
    pdf.set_font('Arial', '', 10)
    pdf.cell(0, 10, f"Tarih: {tarih}", 0, 1, 'R')
    
    cols = ['Sıra'] + list(dataframe.columns)
    w = 150 / len(cols) # Genişlik ayarlı
...
with c1: st.download_button("📊 EXCEL", data=excel_olustur(duzenlenmis_df, genel_str, tarih_metni, secili_sablon, gizle_checkbox), file_name=f"Belge_{dosya_tarihi}.xlsx", type="primary", use_container_width=True)
with c2: st.download_button("📄 WORD", data=word_olustur(duzenlenmis_df, genel_str, tarih_metni, secili_sablon, gizle_checkbox), file_name=f"Belge_{dosya_tarihi}.docx", type="primary", use_container_width=True)
with c3: st.download_button("📕 PDF", data=pdf_olustur(duzenlenmis_df, ara_str, kdv_str, genel_str, tarih_metni, st.session_state.not_alani, kur_metin, secili_sablon, gizle_checkbox), file_name=f"Belge_{dosya_tarihi}.pdf", type="primary", use_container_width=True)
